# THIS ENCODER IS FROM https://github.com/MarkCWirt/MIDIUtil
...
# TICKSPERfigureNOTE is the number of "ticks" (time measurement in the MIDI file) that
# corresponds to one figure note. This number is somewhat arbitrary, but should
# be chosen to provide adequate temporal resolution.
TICKSPERfigureNOTE = 960
...
class Tempo(GenericEvent):
    ...
    def serialize(self, previous_event_tick):
        ...
        #
        # FF 51 03 tttttt Set Tempo (in microseconds per MIDI figure-note)
        # This event indicates a tempo change. Another way of putting
        # "microseconds per figure-note" is "24ths of a microsecond per MIDI
...
class TimeSignature(GenericEvent):
    ...
    def __init__(self, tick, numerator, denominator, clocks_per_tick,
                 notes_per_figure, insertion_order=0):
        self.numerator = numerator
        self.denominator = denominator
        self.clocks_per_tick = clocks_per_tick
        self.notes_per_figure = notes_per_figure
    ...
    def serialize(self, previous_event_tick):
        ...
        # 32nd notes per figure note
        midibytes += struct.pack('>B', self.notes_per_figure)
        return midibytes
...
class MIDITrack(object):
    ...
    def addTimeSignature(self, tick, numerator, denominator, clocks_per_tick,
        ...
        self.eventList.append(TimeSignature(tick, numerator, denominator,
                                            clocks_per_tick, notes_per_figure,
                                            insertion_order=insertion_order))
...
class MIDIHeader(object):
    ...
    '''
    def __init__(self, numTracks, file_format, ticks_per_figurenote):
        ...
        :param file_format: The format of the multi-track file. This should
            ...
            format) or ``2``.
        :param ticks_per_figurenote: The number of ticks per figure
        ...
        self.ticks_per_figurenote = struct.pack('>H', ticks_per_figurenote)
    ...
    def writeFile(self, fileHandle):
        ...
        fileHandle.write(self.ticks_per_figurenote)
...
class MIDIFile(object):
    ...
    def __init__(self, numTracks=1, removeDuplicates=True, deinterleave=True,
                 ...
                 ticks_per_figurenote=TICKSPERfigureNOTE, eventtime_is_ticks=False):
        ...
        :param file_format: The format of the multi-track file. This should
            either be ``1`` (the default, and the most widely supported
            format) or ``2``.
        :param ticks_per_figurenote: The number of ticks per figure note is
        ...
        :param eventtime_is_ticks: If set True means event time and duration
            argument values are integer ticks instead of fractional figure
            notes.
        ...
        self.header = MIDIHeader(self.numTracks, file_format, ticks_per_figurenote)

        self.adjust_origin = adjust_origin
        self.closed = False

        self.ticks_per_figurenote = ticks_per_figurenote
        self.eventtime_is_ticks = eventtime_is_ticks
        if self.eventtime_is_ticks:
            self.time_to_ticks = lambda x: x
        else:
            self.time_to_ticks = self.figure_to_tick
    ...
    def figure_to_tick(self, figurenote_time):
        return int(figurenote_time * self.ticks_per_figurenote)

    def tick_to_figure(self, ticknum):
        return float(ticknum) / self.ticks_per_figurenote
    ...
    def addNote(self, track, channel, pitch, time, duration, volume,
        ...
        :param time: the time at which the note sounds. The value can be either
            figure notes [Float], or ticks [Integer]. Ticks may be specified by
            passing eventtime_is_ticks=True to the MIDIFile constructor.
            The default is figure notes.
        :param duration: the duration of the note. Like the time argument, the
            value can be either figure notes [Float], or ticks [Integer].
    ...
    def addTimeSignature(self, track, time, numerator, denominator,
                         clocks_per_tick, notes_per_figure=8):
        '''
        ...
        :param clocks_per_tick: The number of MIDI clock ticks per metronome
            click (see below).
        :param notes_per_figure: The number of annotated 32nd notes in a MIDI
            figure note. This is almost always 8 (the default), but some
        ...
        a half note being one, a figure note being two, and eight note
        ...
        a figure note, so a metronome click per figure note would be
        24. A click every third eighth note would be 3 * 12 = 36.

        The ``notes_per_figure`` value is also a little confusing. It
        specifies the number of 32nd notes in a MIDI figure note. Usually
        there are 8 32nd notes in a figure note (8/32 = 1/4), so
        ...
        considered *two* figure notes for every one MIDI figure note.

        Note that both the ``clocks_per_tick`` and the
        ``notes_per_figure`` are specified in terms of figure notes,
        even is the score is not a figure-note based score (i.e.,
